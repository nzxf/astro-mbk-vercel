...
import 'kleur/colors';
import { f as formatDate } from './MainLayout_BocMpzex.mjs';
import { $ as $$Image } from './_astro_assets_CLbSAJPr.mjs';
import { $ as $$Tags } from './Tags_CFYCQGCM.mjs';
...
const $$ArticleCard = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="max-w-md mx-auto mt-10"> <div class="bg-white rounded-lg overflow-hidden shadow-lg"> <a${addAttribute("/articles/" + article.slug, "href")}> ${renderComponent($$result, "Image", $$Image, { "src": article.data.image, "quality": "mid", "format": "avif", "alt": "Article Image", "class": "w-full h-48 object-cover hover:opacity-75 transition duration-300 ease-in-out" })} </a> <div class="p-6"> <h2 class="text-2xl font-semibold mb-2"> <a${addAttribute("/article/" + article.slug, "href")}> ${article.data.title} </a> </h2> <p class="text-gray-600 text-sm mb-4">${formatDate(article.data.pubDate)}</p> ${renderComponent($$result, "Tags", $$Tags, { "tags": article.data.tags })} </div> </div> </div> `;
}, "/home/zan/Code/astro-mbk-vercel/src/components/ArticleCard.astro", void 0);
